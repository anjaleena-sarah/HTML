...
<h1 style="margin-top:200px; text-align: center;">Delivery Boy Registration</h1>
...
if (isset($_REQUEST['add'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Insert delivery boy registration
        $qry = "INSERT INTO delivery_boys(first_name, phone_number, email, password, address)
                VALUES ('$name', '$phone', '$email', '$password', '$address')";
        mysqli_query($con, $qry);

        // Insert login details for the delivery boy
        $lqry = "INSERT INTO login(`reg_id`, `username`, `password`, `l_type`)
                 VALUES ((SELECT MAX(seller_id) FROM delivery_boys), '$email', '$password', 'delivery')";
