...
<body>
    ...
    <div class="main-container">
        <div class="left-container">
            ...
            <!--header -->
            <div class="header">
                <a href="seller_home.php" style="font-size: x-large;">
...
<div class="chat-list" id="chatList">
    ...
    ?>
        <a href="Chat.php?id=<?php echo $rs['lid'] ?>&name=<?php echo $rs['username'] ?>&uid=<?php echo $uid?>" style="text-decoration: none;">
            <div class="chat-box">
                <div class="img-box">
    ...
    ?>
        ...
        <div class="right-container">
            <?php
            $iid="select * from login where reg_id='$uid'and l_type='seller'";
            ...
            if (!empty($id)) {
                $id=$_GET["id"];
                $type=$_SESSION['type'];
                // $res = mysqli_query($con, "SELECT `auction_seller_registration`.*,`chat_messages`.* FROM `auction_seller_registration`,`chat_messages` WHERE `chat_messages`.`sender_id`='$uid' AND `chat_messages`.`sender_id`=`auction_seller_registration`.`seller_id` AND `chat_messages`.`sender_id`='$uid'");
            ...
            ?>
                <div class="header">
                    ...
                    <div class="img-text">
                        <div class="user-img">
                            <img class="dp" src="img/profile.png" alt="">
                        </div>
                        <h4><?php
                            if (!empty($name)) {
                                echo $name;
                ...
                <div class="chat-container">
                    ...
                    ?>
                            <div class="message-box my-message">
                                <p style="display: flex;flex-wrap: nowrap;"><?php echo $rs1['message_text'] ?>
                ...
                <form action="" method="post">
                    ...
                    <div class="chatbox-input">
                        ...
                        <button type="submit" name="submit" style="border: none;outline: none;margin-right:10px ;">
                            <!-- <img width="30" height="30" src="/static/images/send-message.png" style="background-color: #009688;border-radius: 100px;" alt="sent"/> -->
  ...
  <script>
    function filterChats(searchQuery) {
      ...
      // Loop through each chat box and hide/show based on search query
      for (const chatBox of chatBoxes) {
        const chatName = chatBox.querySelector('h4').textContent.toLowerCase();
        // If the chat name includes the search query, show the chat box, else hide it
        if (chatName.includes(searchQuery.toLowerCase())) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        $qryReg = "INSERT INTO `chat_messages`(`sender_id`,`receiver_id`,`message_text`,`type`) VALUE ('$latid', '$id','$Message','seller')";

        echo $qryReg ;
        echo"<script>alert('$qryReg');</script>";

        if ($con->query($qryReg) == TRUE) {
            echo "<script>window.location = 'Chat.php?id=$id&name=$name';</script>";
        } else {
            // echo "<script>alert('Message not send');window.location = 'Chat.php';</script>";
        }
    }
